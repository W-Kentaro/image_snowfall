...
if(CONFIG_USER.outputDirectory){
  CONFIG_PATH.sourceBuild = production_mode ? CONFIG_USER.outputDirectory : CONFIG_PATH.dev;
}else{
...
CONFIG = {
  outputDirectory: {
    dev: production_mode ? CONFIG_PATH.sourceBuild : CONFIG_PATH.dev,
    assets: CONFIG_PATH.sourceBuild + '/assets/',
    release: CONFIG_PATH.release
  },
  watchDirectory: {
    html: CONFIG_PATH.sourceBuild + '**/*.html',
    ejs: CONFIG_PATH.source + '**/*.ejs',
    img: CONFIG_PATH.source + '**/img/**/*',
...
const reSet = () => {
  ...
  CONFIG = {
    outputDirectory: {
      dev: production_mode ? CONFIG_PATH.dev : CONFIG_PATH.sourceBuild,
      assets: CONFIG_PATH.sourceBuild + '/assets/',
      release: CONFIG_PATH.release
    },
    watchDirectory: {
...
if(param['--develop']){
  ...
  CONFIG.outputDirectory.dev = CONFIG_PATH.sourceBuild;
  reSet();
  notifier.notify({
    title: 'develop mode',
    message: new Date(),
...
if(param['--production']){
  ...
  CONFIG.outputDirectory.dev = CONFIG_PATH.dev;
  reSet();
 ...
 */
gulp.task('js_lint', () => {
  let _target = CONFIG.watchIgnoreDirectory.js_lint.slice();
...
gulp.task('watch', ['server'], () => {
  ...
  gulp.watch(CONFIG.watchDirectory.webpack, ['webpack']);
...
gulp.task('webp', () => {
  gulp.src(CONFIG.watchDirectory.img)
    .pipe(webp())
    .pipe(gulp.dest(CONFIG.outputDirectory.dev));
});
...
gulp.task('default', (callback) => {
  if(!taskListAdd.length){
    return runSequence(['ejs', 'pug', 'sass', 'js_babel', 'webpack'], 'watch', callback);
  }else{
    return runSequence(['ejs', 'pug', 'sass', 'js_babel', 'webpack'], taskListAdd, 'watch', callback);
  }
});
...
gulp.task('release', (callback) => {
  if(!taskListAdd.length){
    return runSequence(['ejs', 'pug', 'sass', 'js_babel', 'webpack'], 'deploy', callback);
  }else{
    return runSequence(['ejs', 'pug', 'sass', 'js_babel', 'webpack'], taskListAdd, 'deploy', callback);
  }
});
